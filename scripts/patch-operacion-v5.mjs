...
function separarCiudadNacimiento(valor) {
  ...
  if (conParentesis) {
    ...
    const m = normalizar(municipio), d = normalizar(departamento), p = normalizar(pais);
    if (['BOGOTA','BOGOTA D C'].includes(m) && d === 'BOGOTA D C' && p === 'COLOMBIA') {
      municipio = 'BOGOTÁ'; departamento = 'BOGOTÁ D.C.'; pais = 'COLOMBIA';
    }
    return {
      original, municipio, departamento, pais,
      opcionEsperada: departamento ? `${municipio} (${departamento}, ${pais})` : `${municipio} (${pais})`
    };
  }

  const departamentos = [
    'AMAZONAS','ANTIOQUIA','ARAUCA','ARCHIPIELAGO DE SAN ANDRES PROVIDENCIA Y SANTA CATALINA','ATLANTICO','BOGOTA D C',
    'BOLIVAR','BOYACA','CALDAS','CAQUETA','CASANARE','CAUCA','CESAR','CHOCO','CORDOBA','CUNDINAMARCA','GUAINIA','GUAVIARE',
  ...
  for (const dep of departamentos) {
    ...
    if (muni) return { original, municipio:muni, departamento:dep, pais:'COLOMBIA', opcionEsperada:`${muni} (${dep}, COLOMBIA)` };
  }

  const partes = original.split(',').map(x=>x.trim()).filter(Boolean);
  if (partes.length >= 3) {
    const municipio=partes[0], departamento=partes[1], pais=partes.slice(2).join(', ');
    return { original, municipio, departamento, pais, opcionEsperada:`${municipio} (${departamento}, ${pais})` };
  }
  if (partes.length === 2) {
    return { original, municipio:partes[0], departamento:'', pais:partes[1], opcionEsperada:`${partes[0]} (${partes[1]})` };
  }
  return { original, municipio:original, departamento:'', pais:'COLOMBIA', opcionEsperada:'' };
}

`;
  biofile = reemplazarEntre(biofile, 'function separarCiudadNacimiento(valor) {', 'async function visible(locator) {', separador, 'separarCiudadNacimiento');

  const metodo = `  async #seleccionarCiudadNacimiento(locator, valor, etiqueta) {
    ...
    await locator.evaluate(el=>{el.setAttribute('autocomplete','off');el.setAttribute('autocorrect','off');el.setAttribute('spellcheck','false');}).catch(()=>{});
    ...
    const escribir = async(ciudad)=>{
      ...
      await locator.fill('');
      await locator.pressSequentially(normalizar(ciudad)==='BOGOTA D C'?'BOGOTA':String(ciudad),{delay:90});
      await this.page.waitForTimeout(700);
    };

    const coincideCiudad = (valorFinal, ciudad) => {
      const v=normalizar(valorFinal), c=normalizar(ciudad);
      if (!v || !c) return false;
      const cabecera=normalizar(String(valorFinal).split('(')[0].split(',')[0]);
      return cabecera===c || cabecera.includes(c) || c.includes(cabecera);
    };
    ...
    const intentar = async(ciudad, fallback=false) => {
      ...
      const encontrado = await this.page.evaluate(({ciudad,paises,departamento,marca,esCO})=>{
        const norm=v=>String(v||'').normalize('NFD').replace(/[\\u0300-\\u036f]/g,'').replace(/[^a-zA-Z0-9]+/g,' ').trim().toUpperCase();
        const visible=el=>!!(el&&(el.offsetWidth||el.offsetHeight||el.getClientRects().length));
        const c=norm(ciudad), deps=norm(departamento), ps=(paises||[]).map(norm).filter(Boolean);
        const nodos=[...document.querySelectorAll('li,a,button,[role="option"],.dropdown-item,.ui-menu-item,.tt-suggestion,.autocomplete-suggestion,div,span,td')];
        ...
        for(const el of nodos){
          ...
          const n=norm(txt); const cabeza=norm(txt.split('(')[0].split(',')[0]);
          let score=0;
          if(cabeza===c)score+=120; else if(cabeza.startsWith(c)||c.startsWith(cabeza))score+=90; else if(n.includes(c))score+=55; else continue;
          const paisOk=ps.some(p=>p&&n.includes(p));
          if(paisOk)score+=70;
          if(esCO&&deps&&n.includes(deps))score+=35;
          const tag=el.tagName; if(tag==='LI'||el.getAttribute('role')==='option')score+=20; else if(tag==='A'||tag==='BUTTON')score+=14; else if(tag==='DIV')score+=4;
          score-=Math.min(el.childElementCount,8)*2;
          vistos.add(txt); candidatos.push({el,txt,score,paisOk});
        }
        candidatos.sort((a,b)=>b.score-a.score||a.txt.length-b.txt.length);
        const mejor=candidatos.find(x=>x.score>=(esCO?120:170)) || candidatos.find(x=>x.paisOk&&x.score>=135) || candidatos.find(x=>x.score>=120);
      ...
      },{ciudad,paises,departamento,marca,esCO:esColombia(pais)}).catch(()=>({texto:'',opciones:[]}));

      if(encontrado.texto){
        await this.page.locator(`[data-biofile-lugar-opcion="${marca}"]`).first().click({force:true}).catch(()=>{});
        await this.page.waitForTimeout(450);
        const final=await leer();
        if(coincideCiudad(final,ciudad)) return {ok:true,final,opcion:encontrado.texto,fallback};
      }

      for(let pos=0;pos<=12;pos+=1){
        await escribir(ciudad); await locator.focus().catch(()=>{});
        for(let i=0;i<pos;i+=1){await this.page.keyboard.press('ArrowDown').catch(()=>{});await this.page.waitForTimeout(55);}
        await this.page.keyboard.press('Enter').catch(()=>{}); await this.page.waitForTimeout(350);
        const final=await leer();
        if(coincideCiudad(final,ciudad)){
          const fn=normalizar(final); const paisOk=paises.some(p=>fn.includes(p));
          if(paisOk || !final.includes('(')) return {ok:true,final,opcion:final,fallback};
        }
      }
      return {ok:false,opciones:encontrado.opciones||[]};
    };

    let resultado=null; let opcionesVistas=[];
    for(const ciudad of ciudadesSolicitadas){
      const r=await intentar(ciudad,false); opcionesVistas=r.opciones||opcionesVistas; if(r.ok){resultado=r;break;}
    }

    if(!resultado){
      const capital = esColombia(pais) ? capitalDepartamentoColombia(lugar.departamento) : capitalPais(pais);
      if(capital && !ciudadesSolicitadas.some(c=>normalizar(c)===normalizar(capital))){
        const r=await intentar(capital,true); opcionesVistas=r.opciones||opcionesVistas; if(r.ok)resultado=r;
      }
    }
    ...
    await locator.evaluate(el=>{el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));el.dispatchEvent(new Event('blur',{bubbles:true}));}).catch(()=>{});
    const datosLog={valorGoogleSheets:lugar.original,municipioOriginal:lugar.municipio,pais,departamento:lugar.departamento||'',valorFinal:resultado.final,opcionSeleccionada:resultado.opcion};
...
`;
  biofile = reemplazarEntre(biofile, '  async #seleccionarCiudadNacimiento(locator, valor, etiqueta) {', '  async #seleccionarAutocompletado(locator, valor, etiqueta) {', metodo, '#seleccionarCiudadNacimiento');
  fs.writeFileSync(biofilePath,biofile,'utf8');
}

// ==================== GOOGLE SHEETS: ATRIBUCION Y ELIMINADOS ====================
...
if(!sheets.includes('/* ESTADOS_OPERACION_V5 */')){
  ...
  sheets=sheets.replace(colMarker,"  'MODO_INGRESO_BIOFILE',\n  /* ESTADOS_OPERACION_V5 */\n  'REGISTRADO_POR_BIOFILE',\n  'ELIMINADO_POR',\n  'FECHA_ELIMINADO_ISO',\n  'MOTIVO_ELIMINADO'\n];");
  ...
  const manualNuevo=`  async marcarCompletadoManual(documento, usuarioResponsable = '', registradoPor = '') {
...
`;
  ...
  const eliminado=`  async marcarEliminado(documento, actor = '', motivo = '') {
    const row = this.#filaPorDocumento(documento);
    if (!row) throw new Error('No se encontró ese documento en Google Sheets.');
    ...
    return { row, documento: this.#get(row, 'N° documento'), eliminadoPor: actor, motivo: String(motivo || '') };
  }
...
async function resolverUsuarioResponsable(actor, solicitado = '') {
  ...
  if(actor.rol!=='superadmin'){
    if(normalizarUsuario(nombre)!==normalizarUsuario(actor.usuario)) throw new Error('Solo un Super Admin puede atribuir un ingreso manual a otro usuario.');
    return actor.usuario;
  }
  const candidatos=[...config.usuariosEntorno.map(u=>({usuario:u.usuario,activo:u.activo!==false})),...(usuariosStore.disponible()?await usuariosStore.listar():[])];
...
`;
  ...
  const bloque=`  if (req.method === 'POST' && url.pathname === '/api/registros/marcar-manual') {
    const body = await leerJson(req);
    const documento = String(body.documento || '').trim();
    if (!documentoValido(documento)) {
      responderJson(req, res, 400, { ok:false, error:'Documento no válido.' }); return;
    }
    try {
      ...
      responderJson(req,res,200,{ok:true,documento,fila:resultado.row,usuario:usuarioPublico(usuario),atribuidoA:responsable,registradoPor:usuario.usuario,modo:'MANUAL'});
    } catch(error){ responderJson(req,res,400,{ok:false,error:error.message}); }
    return;
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/registros/eliminar') {
    ...
    if(!documentoValido(documento)){responderJson(req,res,400,{ok:false,error:'Documento no válido.'});return;}
    const activo=[...jobs.values()].find(j=>j.documento===documento&&['en_cola','procesando'].includes(j.estado));
    if(activo){responderJson(req,res,409,{ok:false,error:'Este paciente está en cola o procesándose. Espere a que termine antes de enviarlo a Eliminados.'});return;}
    try{
      const base=await cargarBase(); const resultado=await base.marcarEliminado(documento,usuario.usuario,motivo);
